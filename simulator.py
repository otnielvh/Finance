...
def growth_score(income_list: List[data.IncomeCol]) -> float:
    '''
    ...
    for i in range(len(income_list)):
        try:
            score += (float(income_list[i+1].Revenue) - float(income_list[i].Revenue)) / float(income_list[i].Revenue)
        except IndexError:
            pass
            ...
            pass
    return score / (len(income_list) - 1)
...
def get_growth_score(ticker_list: List[str], today: datetime):
    return get_ticker_score(ticker_list, today - relativedelta(years=DEFAUTL_YEARS_BACK), today, score_fun=growth_score)
